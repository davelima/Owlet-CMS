 ...
 * - Postal code CEP (C�digo de Endere�amento Postal)
...
class Validate_ptBR extends Validate
{
     ...
     *            pt_BR CEP/postalCode to validate
     * @param bool $strong
     *            optional; strong checks (e.g. against a list
    ...
    public static function cpf($cpf)
    {
        ...
        if (strlen($cpf) != 11) {
            ...
            $sum = 10 * $number[0] + 9 * $number[1] + 8 * $number[2] + 7 * $number[3] + 6 * $number[4] + 5 * $number[5] + 4 * $number[6] + 3 * $number[7] + 2 * $number[8];
            ...
            if ($result1 == $number[9]) {
                $sum = $number[0] * 11 + $number[1] * 10 + $number[2] * 9 + $number[3] * 8 + $number[4] * 7 + $number[5] * 6 + $number[6] * 5 + $number[7] * 4 + $number[8] * 3 + $number[9] * 2;
                $sum -= (11 * (intval($sum / 11)));
    ...
    public static function cnpj($cnpj)
    {
        ...
        if (strlen($cnpj) != 14) {
            ...
            $sum = $number[0] * 5 + $number[1] * 4 + $number[2] * 3 + $number[3] * 2 + $number[4] * 9 + $number[5] * 8 + $number[6] * 7 + $number[7] * 6 + $number[8] * 5 + $number[9] * 4 + $number[10] * 3 + $number[11] * 2;
            ...
            if ($result1 == $number[12]) {
                $sum = $number[0] * 6 + $number[1] * 5 + $number[2] * 4 + $number[3] * 3 + $number[4] * 2 + $number[5] * 9 + $number[6] * 8 + $number[7] * 7 + $number[8] * 6 + $number[9] * 5 + $number[10] * 4 + $number[11] * 3 + $number[12] * 2;
                $sum -= (11 * (intval($sum / 11)));
                if ($sum == 0 || $sum == 1) {
                    $result2 = 0;
                } else {
     ...
     * @param string $number
     *            phone to validate
     * @param bool $requireAreaCode
     *            require the area code? (default: true)
     *            
