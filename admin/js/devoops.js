...
//
/*
 * function LoadAjaxContent(url){ $('.preloader').show(); $.ajax({ mimeType:
 * 'text/html; charset=utf-8', // ! Need set mimeType only when run from local
 * file url: url, type: 'GET', success: function(data) {
 * $('#ajax-content').html(data); $('.preloader').hide(); }, error: function
 * (jqXHR, textStatus, errorThrown) { alert(errorThrown); }, dataType: "html",
 * async: false }); }
 */
//
...
function loadBlogViewsGraph(post, initDate, finalDate, container) {
    $.ajax({
        url : "ajax/postgraph.json",
        dataType: "JSON",
        data : {
            id : post,
            initDate : initDate,
        ...
        success : function(data) {
            ...
            if(!data.error){
                var lineChartData = {
                    labels : data.table.days,
                    datasets : [ {
                        label : "Visualizações",
                        fillColor : "rgba(0,0,50,0.2)",
                        strokeColor : "rgba(0,0,50,1)",
                        pointColor : "rgba(0,0,50,1)",
                        pointStrokeColor : "#fff",
                        pointHighlightFill : "#fff",
                        pointHighlightStroke : "rgba(0,0,50,1)",
                ...
                var errorMessage = "";
                $.each(data.error, function(i, obj){
                    errorMessage += (obj + "<br>");
                });
                alert(errorMessage);
            }
...
$(document).ready(
        function() {
            ...
            $('.delete').on('click', function(e) {
                e.preventDefault();
                var confirmacao = "", href = $(this).attr('href');
            ...
            if ("tinymce" in $(window)) {
                tinymce.init({
                    ...
                    plugins : [ "advlist autolink lists link image charmap print preview hr anchor pagebreak", "searchreplace wordcount visualblocks visualchars code fullscreen",
                            "insertdatetime media nonbreaking save table contextmenu directionality responsivefilemanager", "emoticons template paste textcolor codemirror" ],
                    relative_urls : false,
                    ...
                    codemirror : {
                        indentOnInit : true, // Whether or
                        // not to indent
                        // code on init.
                        path : '../codemirror', // Path to
                        // CodeMirror
                        // distribution
                        ...
                        jsFiles : [ // Additional JS files to
                        // load
                        'mode/clike/clike.js', 'mode/php/php.js' ]
                    },
                    image_advtab : true,
            ...
            $('.table-banners tbody').sortable({
                ...
                stop : function(a, b) {
                    ...
                    var id = b.item.attr('data-id'), newPosition = b.item.index();
                    $.ajax({
            ...
            if ("mask" in $(window)) {
                ...
                $('.mask').each(function() {
                    var mask = $(this).attr('data-mask'), reverse = $(this).attr('data-reverse') ? true : false;
                    if (mask == "CelSP") {
                        $(this).mask(CelSP, {
            ...
            $('textarea[maxlength]').each(function(i, obj) {
                ...
                $(obj).bind('keyup', function() {
                    var length = $(this).val().length, charsLeft = maxLength - length;
            ...
            $('#updatePostGraph').on('click', function(e){
                e.preventDefault();
                var initDate = $('input[name=initdate]').val();
                var finalDate = $('input[name=finaldate]').val();
                var id = $('input[name=id]').val();
                loadBlogViewsGraph(id, initDate, finalDate, 'linechart');
            });
        });